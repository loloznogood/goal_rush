...
class CatalogueStandardDAO extends AbstractMetierDAO implements DAOInterface
{
    /**
    ...
    public static function getInstance(DatabaseController $dbCtrl)
    {
        if(is_null(self::$instance)){
            self::$instance = new CatalogueStandardDAO($dbCtrl);
        }
    ...
    public function find($pk)
    {
        ...
        if(!$ret){
            ...
            $result = new CatalogueStandard($ret['id'], $ret['lp_id'], $ret['lp'], $ret['ga'], $ret['gp'], $ret['type'], $ret['intitule'], $ret['date_maj'], $ret['fichier_pdf'], $ret['fichier_magazine'], $ret['actif_pdf'], $ret['actif_magazine']);
        }
        return $result;
    }
     ...
     * @return mixed
     */
    public function findCatalogueStandardEntier()
    {
        $ret = $this->getDatabaseCtrl()->queryFetch(
            "SELECT * FROM ".self::TABLE." WHERE type = 'entier' ORDER BY date_maj DESC LIMIT 1"
        );
        if(!$ret){
            $result = $ret;
        }
    ...
    public function findAll()
    {
        ...
        $result = array();
        foreach( $ret as $v){
            $result[]= new CatalogueStandard($v['id'], $v['lp_id'], $v['lp'], $v['ga'], $v['gp'], $v['type'], $v['intitule'],$v['date_maj'], $v['fichier_pdf'], $v['fichier_magazine'], $v['actif_pdf'], $v['actif_magazine']);
        }
        return $result;
    }
    ...
    {
        $ret = $this->getDatabaseCtrl()->queryFetch("SELECT * FROM ".self::TABLE." WHERE lp = :lp  AND ga IS NULL AND gp IS NULL ORDER BY date_maj DESC LIMIT 1", ['lp' => $lp]);
        if(!$ret){
            $result = $ret;
        }
        else{
    ...
    {
        $ret = $this->getDatabaseCtrl()->queryFetch("SELECT * FROM ".self::TABLE." WHERE lp_id = :lpId  AND ga IS NULL AND gp IS NULL ORDER BY date_maj DESC LIMIT 1", ['lpId' => $lpId]);
        if(!$ret){
            $result = null;
        }
        else{
    ...
    public function findAllWithLp($lp)
    {
        $ret = $this->getDatabaseCtrl()->queryFetchAll(
            "
                    SELECT cs.* FROM jf_catalogue_standard cs 
                    INNER JOIN( 
                        SELECT lp, ga, gp, MAX(date_maj) as maxDate 
                        FROM jf_catalogue_standard 
                        WHERE type = 'parGP' 
                        GROUP BY lp, ga, gp
                    ) cs2 ON cs.lp = cs2.lp AND cs.ga = cs2.ga AND cs.gp = cs2.gp  AND cs.date_maj = cs2.maxDate AND cs.lp = :lp
                ", ['lp' => $lp]);
        $result = array();
        foreach( $ret as $v){
            $result[]= new CatalogueStandard($v['id'], $v['lp_id'], $v['lp'], $v['ga'], $v['gp'], $v['type'], $v['intitule'],$v['date_maj'], $v['fichier_pdf'], $v['fichier_magazine'], $v['actif_pdf'], $v['actif_magazine']);
